#!/bin/sh
# agentmemory first-boot entrypoint (adapted from rohitg00/agentmemory deploy/coolify, Apache-2.0).
# Kodra change: when AGENTMEMORY_SECRET is provided by the environment it is used as-is;
# otherwise a secret is generated on first boot and persisted in /data/.hmac, as upstream.
#
...
if [ -n "${AGENTMEMORY_SECRET:-}" ]; then
  : # operator-provided secret; nothing to generate
